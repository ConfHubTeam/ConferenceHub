 ...
 * Provides detailed information about the SMS account
 */
...
class EskizAccountChecker {
  ...
  async checkSenderIDs() {
    try {
      console.log("\n📱 Checking available sender IDs...");
      
      const response = await axios.get(`${this.baseURL}/user/get-sender-names`, {
        headers: {
          'Authorization': `Bearer ${this.token}`,
          'Content-Type': 'application/json'
        }
      ...
      console.log("📋 Sender IDs Response:", JSON.stringify(response.data, null, 2));
      return response.data;
    } catch (error) {
      console.error("❌ Sender IDs check failed:");
      if (error.response) {
        console.error("Status:", error.response.status);
        console.error("Data:", JSON.stringify(error.response.data, null, 2));
      } else {
