...
export default function PolicyFilterModal({ isOpen, onClose }) {
  ...
  const modalContent = (
    <div className="modal-overlay">
      ...
      <div className="modal-container max-w-sm sm:max-w-md md:max-w-2xl lg:max-w-3xl">
        
        {/* Modal Header - Fixed */}
        <div className="modal-header px-2 sm:px-3 py-1.5 sm:py-2">
          <div className="flex items-center gap-2">
            <div className="w-5 h-5 sm:w-6 sm:h-6 bg-accent-primary/10 rounded-full flex items-center justify-center">
              <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-3 h-3 sm:w-4 sm:h-4 text-accent-primary">
          ...
          <button
            onClick={handleClose}
            className="w-5 h-5 sm:w-6 sm:h-6 rounded-full bg-bg-secondary hover:bg-border-light flex items-center justify-center transition-colors"
          >
        ...
        <div className="modal-body p-1.5 sm:p-2">
          ...
          <div className="grid grid-cols-2 md:grid-cols-2 gap-1 sm:gap-1.5">
            {relevantPoliciesWithLabels.map((policy) => (
              <label
                key={policy.key}
                className="flex items-start p-1.5 sm:p-2 bg-bg-card hover:bg-bg-secondary border border-border-light rounded-lg cursor-pointer transition-all duration-200 hover:shadow-sm"
              >
                <input
                  type="checkbox"
        ...
        {/* Modal Footer - Fixed */}
        <div className="modal-footer px-2 py-2 sm:px-3 sm:py-3 md:px-4 md:py-3">
