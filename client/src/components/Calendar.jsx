...
export default function Calendar({ 
  ...
  const handleDateClick = (day) => {
    // Don't allow selecting dates outside min/max range
    if ((min && isBefore(day, min)) || (max && isAfter(day, max))) {
      return;
    }

    if (selectingStart) {
      onDateChange(format(day, "yyyy-MM-dd"), null);
      setSelectingStart(false);
    } else {
      // If clicked date is before start date, swap them
      if (start && isBefore(day, start)) {
        onDateChange(format(day, "yyyy-MM-dd"), format(start, "yyyy-MM-dd"));
      } else {
  ...
  // Check if a date is disabled (past dates or outside min/max range)
  const isDisabled = (day) => {
    const today = new Date();
    today.setHours(0, 0, 0, 0); // Reset time to start of day for accurate comparison
    return isBefore(day, today) || (min && isBefore(day, min)) || (max && isAfter(day, max));
  };
      ...
      <div className="grid grid-cols-7 gap-1">
        {days.map((day, idx) => (
          ...
          >
            ...
            {/* Display X for past dates */}
            {isDisabled(day) && day.getMonth() === currentMonth.getMonth() && (
              <span className="absolute inset-0 flex items-center justify-center">
