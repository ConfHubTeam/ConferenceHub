    ...
    <>
      ...
      {blockedDates.length > 0 && (
        <div className="mb-2 flex justify-between items-center">
          <div className="bg-blue-50 text-blue-800 px-3 py-2 rounded-lg flex items-center">
            ...
            <span className="ml-1">{blockedDates.length === 1 ? t('placeCreate.scheduleAvailability.dateSelected') : t('placeCreate.scheduleAvailability.datesSelected', { count: blockedDates.length })}</span>
            <button 
              type="button"
              onClick={() => setBlockedDates([])}
              className="ml-3 text-xs text-blue-600 hover:text-blue-800 underline"
            >
              {t('placeCreate.scheduleAvailability.clearAll')}
            </button>
          </div>
        </div>
      )}
      ...
      <div className="flex items-center mb-3">
        <label className="flex items-center gap-2 cursor-pointer">
          ...
          <span>{t("placeCreate.scheduleAvailability.blockSpecificDates")}</span>
        </label>
      </div>
      ...
      {showBlockSpecificDates && (
        <div className="mb-2 border p-2 rounded-lg">
          <div className="flex items-center justify-between mb-2">
            <h4 className="text-sm font-medium">{t("placeCreate.scheduleAvailability.selectDatesToBlock")}:</h4>
            {blockedDates.length > 0 && (
              <button
                type="button"
                onClick={() => setBlockedDates([])}
              ...
              >
                {t("placeCreate.scheduleAvailability.clearAllBlockedDates")}
              </button>
            )}
          </div>
          <Calendar 
          ...
          {blockedDates.length > 0 && (
            <div className="mt-2 text-sm">
              <span className="font-medium">{blockedDates.length}</span> {blockedDates.length === 1 ? t('placeCreate.scheduleAvailability.dateCurrentlyBlocked') : t('placeCreate.scheduleAvailability.datesCurrentlyBlocked', { count: blockedDates.length })}
            </div>
          )}
        </div>
      )}
