...
const WeekdayBlocker = ({ blockedWeekdays, setBlockedWeekdays }) => {
  ...
  return (
    <div className="mb-4">
      <h4 className="text-sm font-medium text-gray-700 mb-2">{t("places:form.scheduleAvailability.blockWeekdays")}</h4>
      <div className="flex flex-wrap gap-2">
        {weekdays.map((day) => (
          <label key={day.key} className="flex items-center gap-1 p-2 border rounded-lg">
            ...
            />
            <span>{t(`places:form.scheduleAvailability.weekdays.${day.key}`)}</span>
          </label>
        ))}
      </div>
    </div>
