...
export default function AgentReviewsPage() {
  ...
  }, [user, filterRating, startDate, endDate, searchTerm]);
  ...
  const getFilteredReviewsCount = () => {
    return reviews.filter(review => {
      if (filterReported === "reported" && (!review.Reports || review.Reports.length === 0)) return false;
      if (filterReported === "no_reports" && review.Reports && review.Reports.length > 0) return false;
      return true;
    }).length;
  };
